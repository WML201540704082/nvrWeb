<template>
  <div class="vehicle history-container">
      ...
      <el-col :span="20" style="width:100%;padding: 5px 20px;">
        ...
        <div class="mt20" style="width:85%;">
          ...
          <el-col class="filterItem" :span="4" :offset="2">
            <el-col :span="8" class="searchTitle">{{ $t('transportation') }}</el-col>
            <el-col :span="10" class="flexRowBetween spanBox">
              <el-select v-model="filterParams['transportation']" size="mini"
                        @change="filter('transportation', filterParams['transportation'])"
                        style="margin-left:10px"
                        :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                        clearable
              >
                <el-option v-for="(val, key) of nonmotorList.typeList" :key="key" :label="val.label" :value="val.value">
          ...
          <el-col class="filterItem" :span="4" :offset="0">
            <el-col :span="8" class="searchTitle">{{ $t('transportationColor') }}</el-col>
            <el-select v-model="filterParams['transportationColor']" size="mini"
                       @change="filter('transportationColor', filterParams['transportationColor'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
          ...
          <!-- 车牌角度单选 -->
          <el-col class="filterItem" :span="4" :offset="2">
            <el-col :span="8" class="searchTitle">{{ $t('attitude') }}</el-col>
            <el-select v-model="filterParams['attitude']" size="mini"
                       @change="filter('attitude', filterParams['attitude'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
              <el-option v-for="(val, key) of nonmotorList.attitudeList" :key="key" :label="val.label" :value="val.value">
                <i class="iconfont" v-if="val.color" :style="{color: val.color, marginRight:'12px'}">&#xe62d;</i>{{val.label}}
              </el-option>
            </el-select>
          </el-col>
    ...
    <!-- 人体和车辆 -->
  <div class="mainBox_2 record"
       :style="{width: size.contentWidth - 225 + 'px',
...
<script>
import { commonList, nonmotorList } from '../_utils/listMap'
