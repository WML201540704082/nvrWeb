  ...
  <div class="vehicle history-container">
      ...
      <el-col :span="20" style="width:100%;padding: 5px 20px;">
        ...
        <div class="mt20" style="width:85%;">
          ...
          <el-col :span="4" :offset="0" class="filterItem" clearable>
            <el-col :span="8" class="searchTitle" style="">{{ $t('camera') }}</el-col>
            <el-col :span="10" class="flexRowBetween spanBox channel-select">
          ...
          <el-col class="filterItem" :span="4" :offset="0">
            <el-col :span="8" class="searchTitle">{{ $t('type') }}</el-col>
            <el-select v-model="filterParams['type']" size="mini"
                       @change="filter('type', filterParams['type'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
              <el-option v-for="(val, key) of vehicleList.type" :key="key" :label="val.label" :value="val.value">
          ...
          <!-- 联级选择 车辆品牌 -->
          <el-col :span="4" :offset="2" class="filterItem">
            <el-col :span="8" class="searchTitle" style="">{{ $t('brand') }}</el-col>
            <el-cascader 
                         v-model="brand"
                         size="mini"
                         class="cascaderBox"
          ...
          <!-- 车身颜色单选 -->
          <el-col class="filterItem" :span="12" :offset="2">
            <el-col :span="3" class="searchTitle">{{ $t('carColor') }}</el-col>
            <el-select v-model="filterParams['carColor']" size="mini"
                       @change="filter('carColor', filterParams['carColor'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
              <el-option v-for="(val, key) of vehicleList.carColor" :key="key" :label="val.label" :value="val.value">
                <i class="iconfont" v-if="val.color" :style="{color: val.color, marginRight:'12px'}">&#xe62d;</i>{{val.label}}
              </el-option>
            </el-select>
          </el-col>
          <!-- 车牌颜色单选 -->
          <el-col class="filterItem" :span="4" :offset="0">
            <el-col :span="8" class="searchTitle">{{ $t('plateColor') }}</el-col>
            <el-select v-model="filterParams['plateColor']" size="mini"
                       @change="filter('plateColor', filterParams['plateColor'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
              <el-option v-for="(val, key) of vehicleList.plateColor" :key="key" :label="val.label" :value="val.value">
                <i class="iconfont" v-if="val.color" :style="{color: val.color, marginRight:'12px'}">&#xe62d;</i>{{val.label}}
              </el-option>
            </el-select>
          </el-col>
          ...
          <el-col class="filterItem" :span="4" :offset="2">
            <el-col :span="8" class="searchTitle">{{ $t('pose') }}</el-col>
            <el-select v-model="filterParams['pose']" size="mini"
                       @change="filter('pose', filterParams['pose'])"
                       style="margin-left:10px"
                       :style="{width: (size.contentWidth > 1600 ? 260 : 180) + 'px'}"
                       clearable
            >
              <el-option v-for="(val, key) of vehicleList.pose" :key="key" :label="val.label" :value="val.value">
                <i class="iconfont" v-if="val.color" :style="{color: val.color, marginRight:'12px'}">&#xe62d;</i>{{val.label}}
              </el-option>
            </el-select>
          </el-col>
    ...
    <div class="mainBox_2 record" :style="{width: size.contentWidth - 225 + 'px',
      ...
      <div v-for="(item,index) of passData" :key="index"
        ...
        <img :src="item.cropImagePath" class="bodySize img_scale_down" :onerror="errorVehicleImg" @click="imgChoose(item)">
        <div class="passTime">
          <div style="color:#ffffff" class="rowStart">
            <div class="rowColCenter ellipsis">
              <span class="vehicleNo"
                :style="{backgroundColor: item.plateBackColor, color: (item.plateBackColor === 'rgb(240, 240, 240)' || item.plateBackColor === 'rgb(244, 244, 244)') ? '#303133' : ''}">NO.
                {{item.vehicleID}}</span>
            </div>
          </div>
          <div class="mt5">
...
<script>
import { commonList, vehicleList } from '../_utils/listMap'
import {carBrand} from '@/utils/baseData'
...
export default {
  ...
  methods: {
    ...
    filter(type, val) {
      ...
      this.filterParams['vehicleID'] = this.vehicleID;
