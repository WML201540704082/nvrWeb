<template>
  <div>
    <!-- 设备管理 -->
    <div class="algo" style="overflow: auto; height: calc(100vh - 128px); width: calc(100vw - 260px)">
      <canvas
        ref="scene"
      ...
      <div class="algo_top" style="display: flex">
        <el-form :model="algorithm" ref="algorithm-form" :inline="true">
          <el-form-item label="通道选择" prop="channelId">
            <el-select size="small" v-model="algorithm.channelId" @change="changeChannel(algorithm.channelId,'cut')" placeholder="请选择通道" clearable>
              <el-option
      ...
      <div class="algo_bottom">
        <div class="algo_bottom_left">
          <div class="algo_bottom_left-item" style="padding-left: 5px; font-size: 14px; background-color:#6894b4; border-radius: 5px 0 0 5px;">智能事件</div>
          <div class="algo_bottom_left-slide">
            <div style="cursor: pointer;" v-for="item in algoList" :key="item.index" :class="{active:item.index===getIndex}" @click="changeAlgoList(item)" class="algo_bottom_left-item algo_bottom_left-list">
              <span style="margin-left:5px;">{{item.name}}</span>
              <span :class="item.enabled ? 'classA' : 'classB'" style="font-size: 12px;transform:scale(0.8);">{{item.enabled ? '已开启' : '已关闭'}}</span>
            </div>
          </div>
        </div>
        <div class="algo_bottom_right">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="参数配置" name="first">
              <div class="algo_bottom_right_bottom">
                <div class="algo_bottom_right_bottom_left" v-if="picView">
                    <div>
                        <div style="font-size: 13px;color:#1a58dd; display: flex;justify-content: center;margin-top: -20px;padding: 6px;">(点击“识别区域”按钮后,单击左键开始绘制，双击左键可结束绘制。)</div>
                        <!-- 视频截图及操作按钮 -->
                        <div class="algo_bottom_right_bottom_left_top" style="margin:0 auto">
                  ...
                  <div class="algo_bottom_right_bottom_left_bottom" style="margin-top:20px">
                    <el-col :span="24" style="padding-left:3px;width:640px;height:100%">
                      <div style="color:#909399;font-size:12px;height:100%;">
                        <span style="color:#626262; margin-left: 20px; font-size: 13px; font-weight:Medium;">已开通算法通道({{this.algorithmNum.length}})</span>
                        <div style="display:flex; color:#6894b4;height:30px;line-height:30px;padding-left:20px; padding-top: 10px;" v-if="this.algorithmNum.length>0">
                          <span style="width:50%;padding-left:6px;">通道名称</span>
                          <span style="width:50%;padding-left:6px;">url</span>
                        </div>
                        <div style="height:calc(100% - 48px);overflow-y:auto; color:#626262;" v-if="this.algorithmNum.length>0">
                          <div v-for="(item,index) of this.algorithmNum" :key="index" style="width:100%;" class="channelTable">
                            <div style="width:100%;display:flex;height:24px;line-height:24px;padding-left:20px;">
                              <span class="ellipsis" :title="item.name" style="width:50%;padding-left:6px;">{{item.name}}</span>
                              <span class="ellipsis" :title="item.url" style="width:50%;padding-left:2px;">{{item.url}}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div class="algo_bottom_right_bottom_right" v-if="getIndex != 29">
                  <div style="margin:20px 20px 0">
                    <div style="color: #626262;">
                      算法开关 <el-switch style="margin-left:10px" v-model="enabled" @change="changeEnabled"></el-switch>
                    </div>
                  </div>
                  <div style="margin: 20px 20px 0;">
                    <div style="margin-bottom: 10px;color: #626262;">检测区域设置</div>
                    <div v-if="getIndex != 13 && getIndex != 29 && getIndex != 19" style="display:flex;">
                      <div class="identifyClass" style="width:85px;height:26px;border:1px solid #E7EDEF;color: #1a58dd;cursor:pointer;" id="drawArea">
                        <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                          <i class="el-icon-edit" style="color:#1a58dd"></i>识别区域
                        </div>
                      </div>
                      <div class="identifyClass" style="width:85px;height:26px;border:1px solid #E7EDEF;color: #1a58dd;margin-left:10px;cursor:pointer;" id="clearAll">
                        <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                          <i class="el-icon-refresh" style="color:#1a58dd"></i>清空
                        </div>
                        </div>
                    </div>
                  ...
                  <div v-if="getIndex != 19" style="height:45px;line-height:45px;margin-left:20px;color: #626262;">
                    区域参数
                  </div>
                  <!-- 参数设置 -->
                  <el-form
                  ...
                  >
                    ...
                    <div v-if="getIndex == 19">
                      <template>
                        ...
                        <div style="display: flex; margin: 20px 0 0 50px;">
                          <div class="identifyClass identifyClass1" style="width:85px;height:26px;border:1px solid #E7EDEF;color: #17FFFA;cursor:pointer;" id="drawArea" @click="createOutDraw(false)">
                            <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                            <img src="@/assets/img/identify.png" alt=""/>绘制</div>
                          </div>
                          <div class="identifyClass" style="width:85px;height:26px;border:1px solid #E7EDEF;color: #17FFFA;margin-left:10px;cursor:pointer;" id="clearAll" @click="deleteOutDraw">
                            <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                            <img src="@/assets/img/clear.png" alt="">清空</div>
                          </div>
                        </div>
                      </template>
                    ...
                    <button class="button save" style="height:25px;line-height:25px;" :disabled="btnDisabled" @click="submit('form')">保存配置</button>
                  </el-form>
                </div>
            ...
            <el-tab-pane label="布防计划设定" name="second" v-if="getIndex != 1 && getIndex != 3 && getIndex != 9">
...
<script>
import { getChannels, putChannels, getChannelDet, getChannelPic, putChannelTask } from '@/api/systemAPI';
...
export default {
      ...
      handleClick(tab, event) {
        ...
        changeChannel(e, param) {
            ...
            this.algoList = this.algoList.length > 0 ? this.algoList : [{
                ...
                //     index: 12,
                //     name: "特殊人群",
                //     algorithm: "specialPed",
                //     enabled: (this.algoObject.config.specialPed && this.algoObject.config.specialPed.enabled) ? this.algoObject.config.specialPed.enabled : false
                // }, {
                //     index: 13,
                //     name: "代客操作",
                //     algorithm: "stm",
                //     enabled: (this.algoObject.config.stm && this.algoObject.config.stm.enabled) ? this.algoObject.config.stm.enabled : false
                }, {
                    index: 14,
                    name: "徘徊",
                    algorithm: "hover",
                ...
                //     name: "广告张贴",
                //     algorithm: "adPost",
                //     enabled: (this.algoObject.config.adPost && this.algoObject.config.adPost.enabled) ? this.algoObject.config.adPost.enabled : false
                }, {
                  index: 19,
                  name: '越界侦测',
                ...
                //     name: "抢包",
                //     algorithm: "grab",
                //     enabled: (this.algoObject.config.grab && this.algoObject.config.grab.enabled) ? this.algoObject.config.grab.enabled : false
                }, {
                    index: 24,
                    name: "吸烟",
                    algorithm: "smoking",
                ...
                //     name: "坐姿不雅",
                //     algorithm: "recline",
                //     enabled: (this.algoObject.config.recline && this.algoObject.config.recline.enabled) ? this.algoObject.config.recline.enabled : false
                // }, {
                //     index: 27,
                //     name: "人脸聚类",
                //     algorithm: "faceClustering",
                //     enabled: (this.algoObject.config.faceClustering && this.algoObject.config.faceClustering.enabled) ? this.algoObject.config.faceClustering.enabled : false
                // }, {
                //     index: 28,
                //     name: "人体挟持",
                //     algorithm: "hijack",
                //     enabled: (this.algoObject.config.hijack && this.algoObject.config.hijack.enabled) ? this.algoObject.config.hijack.enabled : false
                // }, {
                //     index: 30,
                //     name: "箱包交接",
                //     algorithm: "swapBag",
                //     enabled: (this.algoObject.config.swapBag && this.algoObject.config.swapBag.enabled) ? this.algoObject.config.swapBag.enabled : false
                // }, {
                //     index: 31,
                //     name: "一日两碰箱",
                //     algorithm: "touchBag",
                //     enabled: (this.algoObject.config.touchBag && this.algoObject.config.touchBag.enabled) ? this.algoObject.config.touchBag.enabled : false
                // }, {
                //     index: 29,
                //     name: "客流",
                //     algorithm: "flowAreas",
                //     enabled: this.algoObject.flowAreas ? true : false
                }];
            // 如果在线
            if (this.picView) {
                ...
                this.tableData.forEach((item, index) => {
                    if ((this.getIndex == 0 && item.config.face?.enabled) || (this.getIndex == 1 && item.config.pedestrian?.enabled)
                        ...
                        this.algorithmNum.push({
                            ...
                            url: this.tableData[index].config.url
                        });
                    }
                });
            });
    ...
    watch: {
        ...
        'algorithm.channelId'() {
          this.$nextTick(() => {
            this.$refs.channelIdInit.getTask(this.algorithm.channelId)
          })
        },
        getIndex(value) {
          if(value == 0 || value == 2) {
