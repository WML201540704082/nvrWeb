<template>
    <div class="TimeLine">
        ...
        <span onselectstart="return false" v-show="showLeft" class="tooltip" :style="{ 'left': offsetLeft + 'px' }">{{
            contentLeft }}</span>
        <span onselectstart="return false" v-show="showRight" class="tooltip" :style="{ 'left': offsetRight + 'px' }">{{
            contentRight }}</span>
        <canvas v-if="canvasShow" :id="ID + '_Bg'" class="scene1bg" width="864" height="12" @mousedown="onDown"
            @mouseup="onUp" @mouseleave="onLeave" @mousemove="onMove"></canvas>
        <canvas :id="ID + '_Line'" class="scene1Line" width="864" height="6"></canvas>
    </div>
</template>
<script>
export default {
    ...
    mounted() {
        ...
        this.canvasDomX = this.canvasDom.offsetWidth
        this.canvasDomY = this.canvasDom.offsetHeight
    ...
    methods: {
        ...
        onUp(e) {
            //获取offsetX当 offsetX<0 ;取0，当offsetX>this.canvasW取this.canvasW
            let currnetOffsetX = e.offsetX < 0 ? 0 : e.offsetX > 864 ? 864 : e.offsetX

            // 新增绘制
            if (this.DrawFlag && !this.MoveFlag) {
                let x0 = this.StartX > currnetOffsetX ? currnetOffsetX : this.StartX;
                let w0 = Math.abs(this.StartX - currnetOffsetX);
                this.DrawArr.push({
                    X: x0,
                    W: w0,
                })
            ...
            if (this.MoveFlag && !this.DrawFlag) {
                //20231103-由于 timeToPx方法计算23:59:59 结果为 863.4059 ，不足864，导致存在误差，特殊判断一下
                console.log(this.moveItem.X, this.moveItem.W, this.ScaleFlag)
                if (this.moveItem.X === 0 && this.moveItem.W === 863.4059 && !this.ScaleFlag) {
                    console.log(this.moveItem.X, this.moveItem.W, this.ScaleFlag, 'xxx')
                    //标识进度条已满，不可移动
                    this.DrawArr.push({
                        X: this.moveItem.X,
                        W: this.moveItem.W,
                ...
                // 改变大小
                    ...
                    // 右边
                    if (this.ScaleRight) {
                        let x0 = this.moveItem.X >= currnetOffsetX ? 0 : this.moveItem.X// 右移：左移
                        let w0 = this.moveItem.X >= currnetOffsetX ? 0 : currnetOffsetX - x0
                        if (w0) {
                            this.DrawArr.push({
                                X: x0,
                                W: w0,
                        ...
                        // 左边
                        let x0 = currnetOffsetX >= this.moveItem.X + this.moveItem.W ? 0 : currnetOffsetX// 右移：左移
                        let w0 = currnetOffsetX >= this.moveItem.X + this.moveItem.W ? 0 : this.moveItem.X + this.moveItem.W - x0
                        if (w0) {
                            this.DrawArr.push({
                                X: x0,
                                W: w0,
                    ...
                    let leftNum = this.moveItem.X - (this.StartX - currnetOffsetX) <= 0 ? 0 : this.moveItem.X - (this.StartX - currnetOffsetX);
                    let rightNum = this.moveItem.X + (currnetOffsetX - this.StartX) >= this.canvasDomX - this.moveItem.W ? this.canvasDomX - this.moveItem.W : this.moveItem.X + (currnetOffsetX - this.StartX);
                    let x0 = currnetOffsetX - this.StartX > 0 ? rightNum : leftNum // 右移：左移
        ...
        onLeave(e) {
            ...
            if (this.DownFlag && (e.offsetY < 0 || e.offsetY >= 11 || e.offsetX < 0 || e.offsetX >= 864)) {
                this.onUp(e)
            }
            if ((e.offsetY < 0 || e.offsetY >= 11 || e.offsetX < 0 || e.offsetX >= 864)) {
                this.showLeft = false
                this.showRight = false
            }
        },
            ...
            if (this.DownFlag) {
                ...
                if (this.MoveFlag) {
                    ...
                    if (this.ScaleFlag) {
                        ...
                        console.log(this.moveItem.X, this.moveItem.W, this.ScaleFlag)
                        if (this.moveItem.X === 0 && this.moveItem.W === 863.4059) {
                            //标识进度条已满，不可移动
                            //20231103-由于 timeToPx方法计算23:59:59 结果为 863.4059 ，不足864，导致存在误差，特殊判断一下
                        } else {
                            let x0, y0 = 0, w0, h0 = this.canvasDomY;
