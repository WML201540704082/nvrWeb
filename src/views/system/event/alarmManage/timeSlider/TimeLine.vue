<template>
    <div class="TimeLine">
        ...
        <span onselectstart="return false" v-show="showRight" class="tooltip" :style="{ 'left': offsetRight + 'px' }">{{
            contentRight }}</span>
        <canvas style="box-sizing: border-box;" v-if="canvasShow" :id="ID + '_Bg'" class="scene1bg" :width="canvasW"
            height="12" @mousedown="onDown" @mouseup="onUp" @mouseleave="onLeave" @mousemove="onMove"></canvas>
        <canvas style="box-sizing: border-box;" :id="ID + '_Line'" class="scene1Line" :width="canvasW" height="6"></canvas>
    </div>
</template>
<script>
export default {
    ...
    created() {
        this.canvasW = Math.floor(document.body.clientWidth / 2.3)
    },
    mounted() {
    ...
    methods: {
        ...
        onUp(e) {
            ...
            let currnetOffsetX = e.offsetX < 0 ? 0 : e.offsetX > this.canvasW ? this.canvasW : e.offsetX
            ...
            if (this.MoveFlag && !this.DrawFlag) {
                ...
                if (this.ScaleFlag) {
                    ...
                    console.log('this.DrawArr', this.DrawArr)
                } else {
        ...
        onLeave(e) {
            //新增当鼠标超过this.canvasW 或者小于0逻辑处理
            console.log(this.DownFlag, e.offsetY, e.offsetX, e)
            if (this.DownFlag && (e.offsetY < 0 || e.offsetY >= 11 || e.offsetX <= 0 || e.offsetX >= this.canvasW)) {
                this.onUp(e)
            }
            if ((e.offsetY < 0 || e.offsetY >= 11 || e.offsetX <= 0 || e.offsetX >= this.canvasW)) {
                this.showLeft = false
                this.showRight = false
                this.DownFlag = false
            }
