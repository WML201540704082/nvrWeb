<template>
    <div :class="isFullFlag ? 'is-fullscreen' : 'normal'">
        <div class="preview">
            <div :class="isFullFlag ? 'frame_full' : 'frame'" id="canvas_form">
                <div style="display:flex;flex-wrap: wrap;">
                    <div v-for="(item,index) of page16List" :key="index">
                        <div ref="canvasDom" v-show="index >= (pageNum-1)*division_show_index && index < pageNum*division_show_index" :style="{width:(division_show_index === 1 ? (canvasWidth - 1) : division_show_index === 4 ? (canvasWidth/2 - 2) :
            ...
            <div class="operate_topCenter" :style="{left: isLocal ? `calc(50vw - 108px)` : `calc(50vw - 139px)`}" v-show="!isFullFlag">
                <!-- 单帧 -->
                <div class="icon_item icon_item1" :class="{disabled: $store.getters.getSyncPlayMode || isVideoPaused}" @click="singleClick()">
            ...
            <div class="operate" v-show="!isFullFlag">
                ...
                <div class="operate_center" :style="{left: screenWidth < 1260 ? '470px' : 'calc(50vw - 163px)'}">
                    <div class="svgMain">
                        <el-tooltip effect="light" content="慢放" placement="top" :open-delay="300">
                            <span @click="changeMinus" v-if="$store.getters.getSyncPlayMode">
                                <svg-icon icon="leftFast" class="svgClass" color="#003058db"></svg-icon>
                            </span>
                            <span @click="changeMinus" v-else>
                    ...
                    <div class="svgMain svgMain1">
                        <el-tooltip effect="light" content="快放" placement="top" :open-delay="300">
                            <span @click="changePlus" v-if="$store.getters.getSyncPlayMode">
                                <svg-icon icon="rightFast" class="svgClass" color="#003058db"></svg-icon>
                            </span>
                            <span @click="changePlus" v-else>
                    ...
                    <div class="svgMain">
                        <div class="svgPlay" :style="{border: disablePlay ? '1px solid #003058db' : ''}">
                            <el-tooltip effect="light" :content="isVideoPaused ? '播放' : '暂停'" placement="top" :open-delay="300">
                                <span ref="videoDom" @click="playVideoDebounce">
                                    <svg-icon v-if="isVideoPaused && !disablePlay" icon="suspend" class="svgClass1 svgClass2"></svg-icon>
                                    <svg-icon v-else-if="isVideoPaused && disablePlay" icon="suspend" class="svgClass1 svgClass2" color="#003058db"></svg-icon>
...
export default {
    ...
    beforeDestroy(){
        for (let i = 1; i < 17; i++) {
            if (this['player' + i]) {
                this['player' + i].video.currentTime = 0;
                this['player' + i].video.load();
    ...
    methods: {
        ...
        operationChannel(flg){
            ...
            if(!flg){
               ...
               if(this.$store.getters.getSyncPlayMode){
                    ...
                    this.selectPageList=[];
                    for(let index in this.page16List){
                        if(this.page16List[index].isVideoPaused===false){
                            this.selectPageList.push(this.page16List[index].ref) ;//保留当前正在播放的视频块channelId
                        }
                    }
                    this.selectPageList.length>0 && this.openVideo(this.selectPageList);
                }
            }else{
                console.log('selectPageList',this.selectPageList );
                if(this.selectPageList && this.selectPageList.length>0){
                    this.openVideo(this.selectPageList,flg);
                }
            }
        },
        async openVideo(selectPageList,flg,cnt=0){
            let itemRef=selectPageList[cnt];
            let index=this.page16List.findIndex(it=>it.ref===itemRef);
            console.log('操作开始！',itemRef,this.$refs[itemRef])
            this.$refs.canvasDom[index].click();
            this.$nextTick( async()=>{
                this.$refs[itemRef][0].muted=true;
                //await this.$refs.videoDom.click();
                await this.playVideo();
                if(flg){
                  this.$refs[itemRef][0].currentTime=this.currentTimeObj[itemRef];
                //   let bufferedTimeRanges= this.$refs[this.page16List[index].ref][0].buffered
                ...
                //   }
                    ...
                    this.currentTimeObj[itemRef]=this.$refs[itemRef][0].currentTime;
                }
                console.log('操作完成！',itemRef,this.$refs[itemRef][0].currentTime)
                if(selectPageList.length>cnt+1){
                  setTimeout(() => {
                    this.openVideo(selectPageList,flg,cnt+1);
                  }, 10);
                }else{

                  this.closePrompt=false;
                  this.selectPageList && flg && (this.selectPageList=[]) && (this.currentTimeObj={});
                }
            })
        },
        ...
        /**
         * 
         * @param {Array} greenDateListArray 同步播放通道列表
         * @param {*} syncStartTime0 光标时间
         */
        initGreenDateList(greenDateListArray, syncStartTime0) {
          this.greenDateListArray = greenDateListArray
          this.cursorTime = syncStartTime0
        },
        initColor() {
            this.itemRef = null
            for (let i = 0; i < this.page16List.length; i++) {
        ...
        batchInit(batchInitArray, callback) {
            ...
            canvasRefArray.map(async (canvasRef, canvasRefIndex) => {
                ...
                this['player' + canvasRef.substring(5)] = new FGPlayer({
                    video: this.$refs[canvasRef][0],
                            ...
                            if (data.eof) {
                                ...
                                if (refItem) {
                                    if (this['player' + itemRef.substring(5)]) {
                                        this['player' + itemRef.substring(5)].video.currentTime = 0;
                                        this['player' + itemRef.substring(5)].video.load();
                                    }
                                ...
                                if (this.sessionId === sessionId) {
                                    ...
                                    // console.log('disablePlay',this.disablePlay);
                ...
                this['player' + canvasRef.substring(5)].play(
        ...
        clickCanvas(item) {
            ...
            if (this.greenDateListArray) {
                this.greenDateList = this.greenDateListArray[this.itemRef.substring(5) - 1]
                this.disablePlay = this.greenDateList.findIndex(item=>moment(this.cursorTime).format('YYYY-MM-DD HH:mm:ss') >= item.start && moment(this.cursorTime).format('YYYY-MM-DD HH:mm:ss') <= item.end) == -1
            }
            
            // console.log('disablePlay---',moment(this.cursorTime).format('YYYY-MM-DD HH:mm:ss'));
            // console.log('this.disablePlay',this.disablePlay);
        ...
        playUrl(channelId, sessionId) {
            ...
            // 根据itemRef绑定canvas，更新时间轴需要sessionId, 根据itemRef挂载player
            this['player' + this.itemRef.substring(5)] = new FGPlayer({ 
                video: this.$refs[this.itemRef][0],
                onConnect: () => {
                    this.isLoading = false;
                    if (isReconnect && timestamp > 0) {
                        let params = {
                    ...
                    this.division_show_index === 1 && this.sizeChange()
                },
                onUserData: (data) => {
                    if (data.action == 'userData') {
                        data = data.userData
                        if (data.eof) {
                            ...
                            let isLocal = JSON.parse(localStorage.getItem('isLocal'))
                            if (this.sessionId === sessionId) {
                                // console.log('data.timestamp123',data.timestamp);
            ...
            const token = JSON.parse(localStorage.getItem('user')).token
            this['player' + this.itemRef.substring(5)].play(
                `ws://${HOST}/api/nvr/record/channel/${channelId}/${sessionId}?Authorization=${token}`,
                this.playParams
            );
        },
        sizeChange(w, h) {
            ...
            for (let i = 0; i < this.division_show_index; i++) {
                ...
                // 之前是canvas版本需要用上面注释的代码处理视频宽高，video版本用下方代码处理
                this._videoWidth = sWidth - 2
                this._videoHeight = sHeight - 2
            }
        },
        // 处理遮盖数据
        ...
        playGray() {
          this.playVideo(true)
          this['player' + this.itemRef.substring(5)].video.currentTime = 0
          this['player' + this.itemRef.substring(5)].video.load()
        },
        /**
         * 光标重新拖动到绿色区域的时候
         * 如果之前处于灰色区域，需要重新打开播放开关
         */
        playGreen() {
          if (this.isVideoPaused) {
            this.playVideo(true)
            ...
            if (this.itemRef && this['player' + this.itemRef.substring(5)]) {
                if (this.$store.getters.getSyncPlayMode) {
                    ...
                    this.refList.forEach(item => {
                        ...
                        if (item.greenDateList.find(item2 => Math.max(moment(this.timestamp).valueOf(), moment(item2.start).valueOf())
                            <= Math.min(moment(this.timestamp).valueOf(), moment(item2.end).valueOf()))) {
                ...
                console.log('xxxxxthis.channelId' ,this.channelId,params,this.itemRef)
                ...
                //     }
                // })
                let res= await ctrlRecordVideo(this.channelId,params)
                if (res.data.code === 200000) {
                    !this.closePrompt && !isGray && this.$message.success(!this.isVideoPaused ? '暂停成功' : '开启成功')
                    this.back = false
                    this.$emit('refBackRun',false)
                    this.page16List.forEach((item,index)=>{
                        if (item.ref === this.itemRef) {
            ...
            if (this.$store.getters.getSyncPlayMode) {
                this.refList.map((ref) => {
                    ...
                    if (this['player' + ref.itemRef.substring(5)]) {
                        this['player' + ref.itemRef.substring(5)].video.currentTime = 0;
                        this['player' + ref.itemRef.substring(5)].video.load();
         ...
         */
        changePlus() {
            if (this.speed_show_index == 8 || this.$store.getters.getSyncPlayMode) {
                return
            }
            if (this.$store.getters.getSyncPlayMode) {
         ...
         */
        changeMinus() {
            if (this.speed_show_index == 0 || this.$store.getters.getSyncPlayMode) {
                return
            }
            if (this.$store.getters.getSyncPlayMode) {
                this.refList.map((canvasRef, index) => {
                        let pageCanvasIndex = this.page16List.findIndex(c => c.ref === canvasRef.itemRef);
                        console.log('pageCanvasIndex',this.page16List);
                        this.page16List[pageCanvasIndex].speed_show_index = this.page16List[pageCanvasIndex].speed_show_index - 1
                        this.speed_show_index = this.page16List[pageCanvasIndex].speed_show_index
                        this.speed = this.speedArray[this.speed_show_index].name
                        this.page16List[pageCanvasIndex].speed = this.speed
        ...
        speedClick() {
            ...
            if (this.$store.getters.getSyncPlayMode) {
              ...
              this.refList.map(canvasRef=>{
                ...
                requests.push(ctrlRecordVideo(canvasRef.channelId,params).then(res => {
                    if (res.data.code === 200000) {
                        for (let i = 0; i < this.refList.length; i++) {
                            this['player' + (i+1)].video.playbackRate = Math.pow(2,this.playSpeed);
                        }
                    }
                }))
              })
            ...
            ctrlRecordVideo(this.channelId,params).then(res=> {
                if (res.data.code === 200000) {
                    this['player' + this.itemRef.substring(5)].video.playbackRate = Math.pow(2,this.playSpeed);
